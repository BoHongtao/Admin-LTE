 ...
 * @since 2.0
 */
class AjaxPager extends Widget
{
    public $container;
    ...
    public function run()
    {
        ...
        echo    '<script type="text/javascript">
                    function '.$this->function.'(url,formId){
                        $.ajax({
                            url:url,
                            data:$(\'#\'+formId).serialize(),
                            beforeSend:function(xhr){
                                $(\'#'.$this->container.'\').append(\'<div id="loading" style="width:40px;height:40px;position: absolute;left: 50%;top: 50%;transform: translate(-50%,-50%);"><img src="static/images/loading.gif" /></div>\');
                            },
...
//        return Html::tag('li', Html::a($label, $this->pagination->createUrl($page), $linkOptions), $options);
        return Html::tag('li', Html::a($label, "javascript:".$this->function."('".$this->pagination->createUrl($page)."','".$this->formId."')", $linkOptions), $options);
    }
