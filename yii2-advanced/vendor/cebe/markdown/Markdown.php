...
class Markdown extends Parser {
	...
	protected function consumeParagraph($lines, $current) {
		...
		for($i = $current, $count = count ( $lines ); $i < $count; $i ++) {
			...
			if (isset ( $this->context [1] ) && $this->context [1] === 'list' && ! ctype_alpha ( $line [0] ) && ($this->identifyUl ( $line, $lines, $i ) || $this->identifyOl ( $line, $lines, $i ))) {
				break;
			}
			
			if ($line !== '' && ltrim ( $line ) !== '' && ! ($line [0] === "\t" || $line [0] === " " && strncmp ( $line, '    ', 4 ) === 0) && ! $this->identifyHeadline ( $line, $lines, $i )) {
				$content [] = $line;
			} else {
				break;
			}
