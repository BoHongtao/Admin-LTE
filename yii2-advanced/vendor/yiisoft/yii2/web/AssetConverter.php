...
class AssetConverter extends Component implements AssetConverterInterface
{
     ...
     *
     * You may also use a path alias to specify the location of the commands:
     *
     * ```php
     * [
    ...
    /**
     * Runs a commands to convert asset files.
     * @param string $command the commands to run. If prefixed with an `@` it will be treated as a path alias.
     * @param string $basePath asset base path and commands working directory
     * @param string $asset the name of the asset file
     * @param string $result the name of the file to be generated by the converter commands
     * @return boolean true on success, false on failure. Failures will be logged.
     * @throws \yii\base\Exception when the commands fails and YII_DEBUG is true.
    ...
    protected function runCommand($command, $basePath, $asset, $result)
    {
        ...
        if ($status === 0) {
            ...
            throw new Exception("AssetConverter commands '$command' failed with exit code $status:\nSTDOUT:\n$stdout\nSTDERR:\n$stderr");
        } else {
            Yii::error("AssetConverter commands '$command' failed with exit code $status:\nSTDOUT:\n$stdout\nSTDERR:\n$stderr", __METHOD__);
        }
