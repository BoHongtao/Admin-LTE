 ...
 *
 * ```php
 * $commands = $connection->createCommand('SELECT * FROM post');
 * $posts = $commands->queryAll();
 * $commands = $connection->createCommand('UPDATE post SET status=1');
 * $commands->execute();
 * ```
 *
 ...
 *
 * ```php
 * $commands = $connection->createCommand('SELECT * FROM post WHERE id=:id');
 * $commands->bindValue(':id', $_GET['id']);
 * $post = $commands->query();
 * ```
 *
...
class Connection extends Component
{
    ...
    /**
     * Creates a commands for execution.
     * @param string $sql the SQL statement to be executed
     * @param array $params the parameters to be bound to the SQL statement
     * @return Command the DB commands
     */
