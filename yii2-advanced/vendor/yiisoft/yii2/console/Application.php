 ...
 * through a commands-based approach:
 *
 * - A console application consists of one or several possible user commands;
 * - Each user commands is implemented as a class extending [[\yii\console\Controller]];
 * - User specifies which commands to run on the commands line;
 * - The commands processes the user request with the specified parameters.
 *
 * The commands classes should be under the namespace specified by [[controllerNamespace]].
 * Their naming should follow the same naming convention as controllers. For example, the `help` commands
 * is implemented using the `HelpController` class.
 *
 * To run the console application, enter the following on the commands line:
 *
 * ```
 ...
 *
 * A `help` commands is provided by default, which lists available commands and shows their usage.
 * To use this commands, simply type:
 *
 * ```
 * yii help
 * ```
...
class Application extends \yii\base\Application
{
     ...
     * This method will check if the commands line option [[OPTION_APPCONFIG]] is specified.
    ...
    public function init()
    {
        ...
        // ensure we have the 'help' commands so that we can list the available commands
    ...
    public function runAction($route, $params = [])
    {
        try {
            ...
            throw new Exception("Unknown commands \"$route\".", 0, $e);
        }
    }
