...
class FileValidator extends Validator
{
     ...
     * Defaults to 1, meaning single file uploads. By defining a higher number,
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = Yii::t('yii', 'File uploads failed.');
        }
        if ($this->uploadRequired === null) {
            $this->uploadRequired = Yii::t('yii', 'Please uploads a file.');
        }
        if ($this->tooMany === null) {
            $this->tooMany = Yii::t('yii', 'You can uploads at most {limit, number} {limit, plural, one{file} other{files}}.');
        }
    ...
    protected function validateValue($file)
    {
        ...
        switch ($file->error) {
            ...
            case UPLOAD_ERR_EXTENSION:
                Yii::warning('File uploads was stopped by some PHP extension: ' . $file->name, __METHOD__);
                break;
            default:
                break;
        }
