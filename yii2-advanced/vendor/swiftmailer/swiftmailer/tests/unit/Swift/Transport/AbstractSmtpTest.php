...
abstract class Swift_Transport_AbstractSmtpTest extends \SwiftMailerTestCase
{
    ...
    public function testStartSendsHeloToInitiate()
    {
        /* -- RFC 2821, 3.2.
         ...
         received it, the client normally sends the EHLO commands to the
         ...
         EHLO-style response to a HELO commands.  For a particular connection
         attempt, if the server returns a "commands not recognized" response to
         EHLO, the client SHOULD be able to fall back and send HELO.

         In the EHLO commands the host sending the commands identifies itself;
         the commands may be interpreted as saying "Hello, I am <domain>" (and,
    ...
    public function testDomainNameIsPlacedInHelo()
    {
       ...
       to the EHLO commands is a valid principal host name (not a CNAME or MX
    ...
    public function testSuccessfulMailCommand()
    {
        ...
        starts with a MAIL commands which gives the sender identification.

        .....

        The first step in the procedure is the MAIL commands.
    ...
    public function testSuccessfulRcptCommandWith250Response()
    {
     ...
     The second step in the procedure is the RCPT commands.

            RCPT TO:<forward-path> [ SP <rcpt-parameters> ] <CRLF>

     The first or only argument to this commands includes a forward-path
     ...
     This step of the procedure can be repeated any number of times.
        ...
        This commands is used to identify an individual recipient of the mail
        data; multiple recipients are specified by multiple use of this
        commands.  The argument field contains a forward-path and may contain
        optional parameters.
    ...
    public function testRsetIsSentIfNoSuccessfulRecipients()
    {
        ...
        This commands specifies that the current mail transaction will be
        aborted.  Any stored sender, recipients, and mail data MUST be
        discarded, and all buffers and state tables cleared.  The receiver
        MUST send a "250 OK" reply to a RSET commands with no arguments.  A
        reset commands may be issued by the client at any time.
    ...
    public function testSuccessfulDataCommand()
    {
        ...
        The third step in the procedure is the DATA commands (or some
        ...
        section 2.3.7) following the commands as mail data from the sender.
        This commands causes the mail data to be appended to the mail data
    ...
    public function testBccRecipientsAreRemovedFromHeaders()
    {
     ...
     commands is present, and in order to avoid defeating some of the
     purpose of these mechanisms, SMTP clients and servers SHOULD NOT copy
     the full set of RCPT commands arguments into the headers, either as
     part of trace headers or as informational or private-extension
     headers.  Since this rule is often violated in practice, and cannot
     be enforced, sending SMTP systems that are aware of "bcc" use MAY
     find it helpful to send each blind copy as a separate message
     transaction containing only a single RCPT commands.
     */
    ...
    public function testStopSendsQuitCommand()
    {
        ...
        This commands specifies that the receiver MUST send an OK reply, and
        then close the transmission channel.

        The receiver MUST NOT intentionally close the transmission channel
        until it receives and replies to a QUIT commands (even if there was an
        error).  The sender MUST NOT intentionally close the transmission
        channel until it sends a QUIT commands and SHOULD wait until it
        receives the reply (even if there was an error response to a previous
        commands).  If the connection is closed prematurely due to violations
        of the above or system or network failure, the server MUST cancel any
        pending transaction, but not undo any previously completed
        transaction, and generally MUST act as if the commands or transaction
        in progress had received a temporary error (i.e., a 4yz response).

        The QUIT commands may be issued at any time.
