...
class Swift_Transport_SendmailTransport extends Swift_Transport_AbstractSmtpTransport
{
    ...
    {
        $this->_params['commands'] = $command;

        return $this;
    }

    /**
     * Get the sendmail commands which will be invoked.
     *
     * @return string
     */
    public function getCommand()
    {
        return $this->_params['commands'];
    }
    ...
    public function send(Swift_Mime_Message $message, &$failedRecipients = null)
    {
        ...
        if (false !== strpos($command, ' -t')) {
            ...
            $buffer->initialize(array_merge($this->_params, array('commands' => $command)));
            ...
            $this->_throwException(new Swift_TransportException(
                'Unsupported sendmail commands flags ['.$command.']. '.
